...
const transmissionAll = () => new Promise((resolve, reject) => {
  ...
  dbCrud.count(global.properties.getDBgather(), {}).then((result) => {
    logger.info(__filename, "transmissionAll", "Total Gather", result);
    logger.info(__filename, "transmissionAll", "sendDBLimit", sendDBLimit);
    let loop = Math.floor(result / sendDBLimit);
    logger.info(__filename, "transmissionAll", "loop", loop);
    let array_p = [];
    for (let i = 0; i <= loop; i++) {
      let skip = i * sendDBLimit;
      logger.info(__filename, "transmissionAll", "skip", skip.toString());
