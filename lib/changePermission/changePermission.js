...
const changePermissionWindows = () => new Promise((resolve, reject) => {
  logger.debug(__filename, "changePermissionWindows", global.properties.getAppDir(), global.properties.getAppDir().length);
  if (global.properties.getAppDir().length <= 3) {
    ...
    args.push('/T');
    args.push('/C');
    args.push('/grant ' + global.properties.getUserName() + ":(OI)(CI)(F)");
