...
cronEmitter.on('loadCronDBAllItens', () => {
  ...
  crudDB.find(global.properties.getDBmetrics(), {}).then((result) => {
    logger.debug(__filename, "loadCronDBAllItens", "itens crons", result);
    result.map((item) => {
      logger.debug(__filename, "loadCronDBAllItens", "item", item);
      cronEmitter.emit('includeItemCron', item, item.newItem);
    });
  }).catch((error) => {
    logger.error(__filename, "loadCronDBAllItens", "error", error);
