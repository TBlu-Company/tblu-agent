...
const findMelhorVersao = (arrayVersao) => new Promise((resolve, reject) => {
  logger.debug(__filename, 'findMelhorVersao', 'arrayVersao.length', arrayVersao.length);
  ...
  exec(command, (error, stdout) => {
    if (error) {
      ...
      Promise.all(pall).then(result => {
        logger.debug(__filename, 'pcoluna', 'result', result);
        let cresult = _.compact(result);
        logger.debug(__filename, 'pcoluna', 'cresult', cresult);
        if (cresult.length >= 1) {
          resolve(cresult);
        } else {
          resolve(undefined);
  ...
  testOracleHomeEnv().then((result) => {
    ...
    exec(command, (error, stdout) => {
      if (error) {
        ...
        Promise.all(pall).then(result => {
          let cresult = _.compact(result);
          cresult = _.flattenDeep(cresult);
          logger.debug(__filename, 'getOracleHomeLinux', 'cresult', cresult);
          if (cresult.length >= 1) {
            findMelhorVersao(cresult).then(result2 => {
              logger.debug(__filename, 'getOracleHomeLinux', 'result2', result2);
              resolve(result2);
            });
          } else {
            resolve(undefined);
