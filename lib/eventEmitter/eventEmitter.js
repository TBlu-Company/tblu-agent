...
ctrlEmitter.on('loadDBSendDBCronTime', () => {
  logger.debug(__filename, "ctrlEmitter", "loadDBSendDBCronTime");
  dbloadProperties.loadDBSendDBCronTime().then(() => {
    ctrlEmitter.emit('loadDBSendDBCronTime-end');
  }).catch((error) => {
    ctrlEmitter.emit('error', error);
  });
});
...
ctrlEmitter.on('loadDBLoggerLevel', () => {
  logger.debug(__filename, "ctrlEmitter", "loadDBLoggerLevel");
  dbloadProperties.loadDBLoggerLevel().then(() => {
    ctrlEmitter.emit('loadDBLoggerLevel-end');
  }).catch((error) => {
    ctrlEmitter.emit('error', error);
  });
});
...
ctrlEmitter.on('cronStopAll', () => {
  logger.debug(__filename, "ctrlEmitter", "cronStopAll");
  cron.emit('stopAll');
